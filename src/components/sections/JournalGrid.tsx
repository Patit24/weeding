      ...
      <div className="mb-9 grid gap-4 lg:grid-cols-[1fr_auto]">
        <div className="flex flex-wrap gap-3" role="tablist" aria-label="Journal filters">
          {journalCategories.map((item) => (
            <motion.button key={item} type="button" role="tab" aria-selected={category === item} onClick={() => setCategory(item)} whileHover={{ y: -2 }} whileTap={{ scale: 0.98 }} className={`min-h-11 rounded-full border px-5 text-xs uppercase tracking-[0.16em] transition-colors ${category === item ? "border-[var(--espresso)] bg-[var(--espresso)] text-[var(--warm-ivory)] shadow-sm" : "border-[var(--fine-border)] hover:border-[var(--espresso)]"}`}>
              {item}
            </motion.button>
          ))}
        </div>
        <label className="block">
          ...
          <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search notes" className="min-h-11 w-full rounded-full border border-[var(--fine-border)] bg-transparent px-5 text-sm outline-none lg:w-72" />
        </label>
      </div>
