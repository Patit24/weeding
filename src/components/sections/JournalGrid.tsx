      ...
      <div className="mb-9 grid gap-4 lg:grid-cols-[1fr_auto]">
        <div className="flex flex-wrap gap-3" role="tablist" aria-label="Journal filters">
          {journalCategories.map((item) => (
            <button key={item} type="button" role="tab" aria-selected={category === item} onClick={() => setCategory(item)} className={`min-h-11 border px-4 text-xs uppercase tracking-[0.16em] ${category === item ? "border-[var(--espresso)] bg-[var(--espresso)] text-[var(--warm-ivory)]" : "border-[var(--fine-border)]"}`}>
              {item}
            </button>
          ))}
        </div>
