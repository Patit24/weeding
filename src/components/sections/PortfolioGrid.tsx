      ...
      <div className="mb-10 flex flex-wrap gap-3" role="tablist" aria-label="Portfolio filters">
        {portfolioCategories.map((item) => (
          <motion.button
            ...
            whileHover={{ y: -2 }}
            whileTap={{ scale: 0.98 }}
            className={`min-h-11 rounded-full border px-5 text-xs uppercase tracking-[0.16em] transition-colors ${category === item ? "border-[var(--espresso)] bg-[var(--espresso)] text-[var(--warm-ivory)] shadow-sm" : "border-[var(--fine-border)] hover:border-[var(--espresso)]"}`}
          >
            {item}
          </motion.button>
        ))}
          ...
          {items.map((item, index) => (
            ...
            >
              <Link href={`/portfolio/${item.slug}`} className="group block">
                <span className={`relative block overflow-hidden rounded-2xl ${index === 0 ? "aspect-[16/9]" : "aspect-[4/5]"}`}>
                  ...
                  <span className="absolute inset-0 flex items-end rounded-2xl bg-[rgba(41,35,31,0.56)] p-5 opacity-0 transition-opacity group-hover:opacity-100 group-focus-visible:opacity-100">
